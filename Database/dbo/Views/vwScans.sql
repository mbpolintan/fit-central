...
select 
		...
		on mem.MemberId = chaMem.MemberId
		left join [dbo].[Challenge] cha
		on cha.ChallengeId = chaMem.ChallengeId
		outer apply
		...
		outer apply
		(select top 1 SiteId from [dbo].[Studio] std where mem.StudioId = std.StudioId) as std
		outer apply
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwScans';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwScans';
